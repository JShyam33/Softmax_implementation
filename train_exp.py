...
from models import SoftmaxApprox, ExpApprox
...
def train(model, epochs, batch_size, device):
    ...
    for epoch in range(epochs):
        ...
        x, targets = generate_data(batch_size)
...
def tune_train_func(config, checkpoint_dir=None):
    ...
    input_dim =  1  # Dimension for the input logit vector
    hidden_dims = [7]  # Hidden layer sizes
    epochs = 10000 # Reduced epochs for tuning

    model = ExpApprox(input_dim, hidden_dims, dropout_prob=config["dropout_prob"])
    ...
    for epoch in range(epochs):
        ...
        x, targets = generate_data(config["batch_size"])
        x, targets = x.to(device), targets.to(device)
